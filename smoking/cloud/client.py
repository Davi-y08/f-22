...
class CloudClient:
    def __init__(
        ...
        self._events: queue.Queue[dict[str, Any]] = queue.Queue(maxsize=max(1, event_queue_size))
    ...
    def emit_event_async(self, event: "DetectionEvent") -> None:
        ...
        try:
            ...
            if self.logger:
                self.logger.warning(
                    "cloud_event_queue_full",
                    extra={"agent_id": self.agent_id, "event_id": payload.get("event_id")},
                )
    ...
    def _event_worker(self) -> None:
        while not self._stop_event.is_set() or not self._events.empty():
            try:
                payload = self._events.get(timeout=0.25)
            except queue.Empty:
                continue
            ...
            try:
                ...
                if self.logger:
                    self.logger.warning(
                        "cloud_event_sync_failed",
                        extra={"agent_id": self.agent_id, "event_id": payload.get("event_id"), "error": str(exc)},
                    )
            finally:
    ...
    def _post_json(self, path: str, payload: dict[str, Any]) -> Any:
        ...
        try:
            ...
            raise CloudClientError(f"API retornou HTTP {exc.code}: {_short_error_body(body)}") from exc
