param(
    ...
    [string]$ExeName = "StealthLensDesktopLite",
    [string]$PythonExe = ""
)
...
if (-not (Test-Path "models/smoking_monitor.onnx")) {
    throw "Modelo Lite ausente em models/smoking_monitor.onnx. Rode: python export_lite_model.py"
}
...
$pyinstallerArgs += @("--add-data", "models/smoking_monitor.onnx;models")
...
$distModelPath = Join-Path $distModelsDir "smoking_monitor.onnx"
$internalModelPath = Join-Path $distAppDir "_internal\models\smoking_monitor.onnx"
if (Test-Path $internalModelPath) {
    ...
    Copy-Item -LiteralPath "models/smoking_monitor.onnx" -Destination $distModelPath -Force
}
